...
val verifyGuestAssets by tasks.registering {
    doLast {
        listOf("kernel", "firmware", "initrd", "manifest.json", "disk.seed", "system.seed", "service.seed", "wayland.seed").forEach { name ->
...
val verifyDebugApkAssets by tasks.registering {
    ...
    doLast {
        ZipFile(layout.buildDirectory.file("outputs/apk/debug/app-debug.apk").get().asFile).use { apk ->
            for (name in listOf("disk.seed", "system.seed", "service.seed", "wayland.seed")) {
