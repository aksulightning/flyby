...
class VmService : Service() {
    ...
    fun selectDisk(mode: DiskMode): Boolean {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START -> {
                ...
                scope.launch {
                    val accepted = vm.start(VmConfig()) { GuestResources.prepare(this@VmService, settings.state.value.disk) }
            ...
            ACTION_EXPORT, ACTION_IMPORT -> {
                ...
                val mode = settings.state.value.disk
